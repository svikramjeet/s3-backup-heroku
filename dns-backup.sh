...
AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_BACKUP AWS_SECRET_ACCESS_KEY=$AWS_SECRET_KEY_BACKUP AWS_DEFAULT_REGION=$AWS_REGION_BACKUP /app/vendor/bin/aws s3 cp /tmp/"${FNAME}_${FILENAME}".gz s3://$S3_BUCKET_PATH/$FNAME/"${FNAME}_${FILENAME}".gz --expires $EXPIRATION_DATE
